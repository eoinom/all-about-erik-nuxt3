<template>
  ...
  >
    ...
    >
      <img
        :alt="thumbnailObj.title"
        v-if="thumbnailObj.img != null"
        :src="thumbnailObj.img"
        class="thumbnailImg hideOnHover"
      />
      <img
        :alt="thumbnailObj.title"
        v-if="thumbnailObj.img_bright != null"
        :src="thumbnailObj.img_bright"
...
export default {
  ...
  data() {
    return {
      link: this.thumbnailObj.hasOwnProperty('text')
        ? slugify(this.thumbnailObj.text, { customReplacements: [["'", '']] })
        : '',
      windowWidth: 0,
    };
  },
...
@media only screen and (max-width: 512px) {
  .overlayText {
    ...
    letter-spacing: calc(4.125px + 4.875 * (100vw - 332px) / (512 - 332));
  }
  .thumbnailImgContainer:after {
    --blur_px: calc(
      var(--blur) * 1px * (0.078125rem + 0.75 * (100vw - 332px) / (512 - 332))
    );
    --spread_px: calc(
      var(--blur) * 1px * (0.078125rem + 0.75 * (100vw - 332px) / (512 - 332))
    );
...
@media only screen and (min-width: 768px) and (max-width: 1152px) {
  .overlayText {
    ...
    line-height: calc(
      2.2083333rem + 17.6666667 * (100vw - 768px) / (1152 - 768)
    );
    letter-spacing: calc(4.667px + 4.333 * (100vw - 768px) / (1152 - 768));
  }
  .thumbnailImgContainer:after {
    --blur_px: calc(
      var(--blur) * 1px * (0.08333rem + 0.6667 * (100vw - 768px) / (1152 - 768))
    );
    --spread_px: calc(
      var(--blur) * 1px * (0.08333rem + 0.6667 * (100vw - 768px) / (1152 - 768))
    );
